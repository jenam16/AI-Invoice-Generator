...
export async function getInvoices(req, res) {
    try {
        ...
        console.log("getInvoices error:", err);
        return res.status(500).json({
            success: false,
...
export async function updateInvoice(req,res){
 ...
 const {id} = req.params;
 const body = isObjectIdstring(id) ? {_id:id , owner:userId}:{ invoiceNumber:id,owner:userId}
...
// ensure that it is unique and not already taken by another invoice
    ...
    return res.status(200).json({
        success:true,
        message:"Invoice updated successfully",data:updated,
    })
}
     catch (err) {
